...
if (context.payload.comment.body.trim().toLowerCase().startsWith("!nim ") && (unlockedAllowAll || checkAuthorAssociation()) ) {
  ...
  if (addReaction(githubClient, "eyes")) {
    ...
    let issueCommentStr = `<details><summary><kbd>${ process.env.RUNNER_OS }</kbd> ${ osEmoji } bisect by @${ context.actor } (${ context.payload.comment.author_association.toLowerCase() })</summary>`
